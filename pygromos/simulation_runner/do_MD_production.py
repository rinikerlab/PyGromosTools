...
from pygromos.hpc_queuing import FileManager as fM

from pygromos.hpc_queuing.job_scheduling import module_functions as gjs
from pygromos.hpc_queuing.job_scheduling.schedulers import MD_simulation_scheduler as md

spacer="////////////////////////////////////////////////////////////////////////////////////////////////////////////"


def do(out_root_dir:str, in_simSystem:fM.System, in_template_imd:str, in_gromosXX_bin_dir:str, in_gromosPP_bin_dir:str, in_ene_ana_lib_path:str,
...
int
    ...
    if (protein.number_of_atoms > 0):
        ...
        if (isinstance(non_ligands, type(None))):  # TODO: DIRTY HACK: in PNMT is Cofactor at end of the file.
            ...
            solvent_bath = (ligands.number_of_atoms + protein.number_of_atoms + non_ligands.number_of_atoms + clean_residues[
                "SOLV"])
    ...
    from pygromos.hpc_queuing.job_scheduling.workers.analysis_workers import MD_simulation_analysis as md_ana
    analysis_vars = {"out_analysis_dir": analysis_dir, "in_simulation_dir":sim_dir,
                     "in_topology_path": simSystem.top_path.top_path, "in_simulation_name":simSystem.name, "in_imd_path": imd_out_path,
                     "control_dict":md_ana.template_control_dict,
                     "in_ene_ana_lib_path":in_ene_ana_lib_path, "gromosPP_bin_dir":in_gromosPP_bin_dir, }


    in_analysis_script_path = gjs.write_job_script(out_script_path=out_root_dir + "/job_analysis.py",
                                                   target_function=md_ana.do, variable_dict=analysis_vars, no_reeds_control_dict=True)

    print("generating LSF-Bashscripts")
    print(" nmpi: ", nmpi, "   nomp: ", nomp)

    args_dict = {"in_simSystem":simSystem, "in_imd_path":imd_out_path, "programm_path":in_gromosXX_bin_dir, "out_dir_path":sim_dir,
                 "equilibration_num":equilibration_runs, "simulation_num":simulation_runs, "write_out_free_energy_traj":write_free_energy_traj,
                  "nmpi":nmpi, "nomp":nomp, "duration_per_job":duration, "analysis_script": in_analysis_script_path, "verbose":verbose}

    schedule_jobs_script_path = gjs.write_job_script(out_script_path=out_root_dir + "/production_job.py",target_function=md.do, variable_dict=args_dict)
