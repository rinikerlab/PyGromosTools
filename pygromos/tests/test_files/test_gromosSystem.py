...
class test_gromos_system(unittest.TestCase):
    ...
    def test_construct_files(self):
        subSys_files = self.class_type(work_folder=tmp_test_dir, system_name="Testing1",
                                       in_cnf_path=self.input_cnf_path, in_top_path=self.input_top_path)

        subSys_files2 = self.class_type(work_folder=tmp_test_dir, system_name="Testing2",
                                        in_cnf_path=self.input_cnf_path, in_top_path=self.input_top_path,
    ...
    def setUp(self) -> None:
        self.subSys_files = self.class_type(work_folder=tmp_test_dir, system_name="Testing",
                                            in_cnf_path=self.input_cnf_path, in_top_path=self.input_top_path,
                                            in_imd_path=template_md)
