...
def adapt_imd_template(in_template_imd_path: str, out_imd_dir: str, cnf: cnf_cls.Cnf, exclude_residues: list = [],
                       simulation_steps: int = 100000, solvent_keyword: str = "SOLV"):
    """
    ...
    if (len(exclude_residues) > 0 and not "prot" in residues):
        solvent_bath = (
                lig_atoms + sum([sum(list(residues[x].values())) for x in exclude_residues]) +
    ...
    # Temperature baths
        ...
        if (len(exclude_residues) < 0):  # TODO: DIRTY HACK: in PNMT is Cofactor at end of the file.
            ...
            solvent_bath = (
                    lig_atoms + n_atoms_prot + sum([sum(list(residues[x].values())) for x in exclude_residues]) +
                    residues[solvent_keyword])
...
def write_job_script(out_script_path:str, target_function:callable, variable_dict:dict, python_cmd:str= "python3", verbose:bool=False, no_reeds_control_dict:bool=False)->str:
    ...
    if(len(missed_keys)>0):
        raise ValueError("Found some variables missing in variable dict,that are required!\n\t"+"\n\t".join(missed_keys))
