...
def do(in_simulation_dir:str, out_analysis_dir:str, sim_prefix:str, #in_system:Gromos_System,
        gromosPP_bin_dir:str=None, n_processes: int = 1, control_dict: dict = None, verbose: bool = True):
    """
    ...
    if(control_dict["concat"]["do"]):
        project_concatenation(in_folder=in_simulation_dir, in_prefix = sim_prefix,  out_folder=out_data_dir, control_dict=control_dict["concat"]["sub"], verbose=verbose)
...
def project_concatenation(in_folder: str, out_folder: str, in_prefix:str, #in_simSystem: Gromos_System,
                          control_dict: Dict[str, bool], verbose: bool = False) -> dict:
    ...
    if (control_dict["cp_cnf"]):
        ...
        if(os.path.exists(out_prefix+".cnf")):
            ...
            sim_dir_cnfs = gather_simulation_step_file_paths(in_folder, filePrefix=in_prefix, #in_simSystem.name,
                                                                fileSuffixes=".cnf",
                                                                verbose=verbose)
    ...
    if (control_dict["cat_trc"]):
        ...
        if(os.path.exists(out_traj_path)):
            warnings.warn("Skipping concatenation of trcs, as there already exists one!\n "
                          "If you want to generate a new trc. Please delete the old one!\n\t Path:"+out_traj_path)
        else:
            if (verbose): print("\tStart Trc Cat")
    ...
    if (control_dict["cat_tre"]):
        ...
        if(os.path.exists(out_traj_path)):
            ...
            if (verbose): print("\tStart Tre Cat")
            traj_files = gather_simulation_step_file_paths(in_folder, filePrefix=in_prefix,
...
def gather_simulation_step_file_paths(in_folder: str, filePrefix: str = "",
    ...
    for dirname, dirnames, filenames in os.walk(in_folder):
        ...
        tmp_files = [file for file in filenames if
                     (filePrefix in file and any([file.endswith(suffix) for suffix in fileSuffixes]))]
