...
def write_job_script(out_script_path: str, target_function: callable, variable_dict: dict, python_cmd: str = "python3",
    ...
    from pygromos.hpc_queuing.submission_systems.Submission_Systems import _SubmissionSystem

    missed_keys = []
    for key in s.parameters:
        if (key in variable_dict):
            value = variable_dict[key]
            if (isinstance(value, Gromos_System) or issubclass(value.__class__, _SubmissionSystem)):  # this is a nasty way! ... tends to fail!
                sys = value
