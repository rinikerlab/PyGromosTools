...
class NOE(_general_gromos_file._general_gromos_file):
    ...
    def read_file(self):
        ...
        ###read header info
        ###NOE Restraint
        header = [s.replace("#", "") for s in
                  noe_states[0]["NOE VIOLATIONS"] if (s.startswith("#"))]
        ...
        t_legend = [[int(x.strip().split(" ")[0])] + [y for y in x.strip().split(" ")[1:] if (y != "")] for x in
                    t_legend]
        ...
        return {"TITLE":title, "AVERAGE_NOE":average_NOE, "NOE_VIOLATIONS":NOE_violations, "RESTRAINT_LEGEND":restraint_legend}
...
class JVAL(_general_gromos_file._general_gromos_file):
    ...
    _data_header: dict = {"num": int, "mol": int, "resI": int, "resN": str,
              "atomNI": str, "atomNJ": str, "atomNK": str, "atomNL": str,
              "atomIdI": str, "atomIdJ": str, "atomIdK": str, "atomIdL": str,
              "A": float, "B": float, "C": float,
              "delta": float, "J_0": float, "phi_ave": float, "phi_rmsd": float, "J_ave": float, "J_rmsd": float,
              "|Jave-J0|": float}
