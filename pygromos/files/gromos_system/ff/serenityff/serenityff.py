            ...
            if len(matches) >= 1:
                for match in matches:
                    ...
                    if element == "C":
                        print("----------------")
                        print(idx_in_rdkmol)
                        print(Chem.MolToSmarts(mol_pattern))
                        print("----------------")        
        return return_dict
        ...
        if self.develop:
            ...
            for molecule in self.off2g.molecule_force_list:
                ...
                for key in molecule["vdW"]:
                    ...
                    self.off2g.gromosTop.add_new_atom(ATNM=ATNM, MRES=MRES, PANM=PANM, IAC=IAC, MASS=MASS, CG=CG, CGC=CGC, INE=INE, INE14=INE14, C6=C6, C12=C12, CS6=CS6, CS12=CS12, IACname=IACname)
                moleculeItr += 1
        else:
            raise NotImplementedError("WIP")
