    ...
    for line in file.readlines():
        if (not first_key and not "\#" in line and len(line.split()) == 1):
            ...
            if(line.startswith("#")):
                ...
                raise IOError("coord file has inconsistent gromos Block structure!\n This line is out of block: \n"+line)
        else:
            if(not "#" in line):
...
def read_repdat(path: str,  Vj_header=False) ->(Dict, List[blocks.repdat.replica_stat]):
    ...
    for line in file.readlines():
        ...
        if(line.startswith("#")):
            if(Vj_header):
                data_header = list(map(lambda x: x.split("(")[0], filter(lambda x: not str(x).startswith("Vj"), line.replace("#", "").split())))
            else:
            ...
            if(len(fields)>1):
                key = fields[0].replace(":","").strip().replace("Number of ", "").replace(" ", "_").replace(",_num", "_").replace("=","").replace("_(RE-EDS)", "")
                value = fields[1]
    ...
    if(new_repdat): ### TODO: NEW PARSER for actual gromos version!
        ...
        system_options = blocks.repdat.repex_system(s=system_options_dict["s"], T=system_options_dict["T"], state_eir=system_options_dict["state_eir"])
        ...
        new_dict = {i: system_options[i] for i in system_options if not (i.startswith("eir") or i.strip().startswith("E")) and (i == "s" or i == "T")}
        #print(new_dict.keys())
        if("T" in new_dict):
            ...
            if len(subkeys) == len(values) and len(subkeys) > 1:    #more keys and equally ammount of values
                ...
                # Hack FORCE block - energy groups do not have a parameter name per parameter
                    ...
                    if not ((len(values)-1) == int((values[0]))):
                        print("Warning - In FORCE block: Number of energy groups does not match number of last atoms given!")
    ...
    with open(in_file_path, 'r') as infile:
        for line in infile:
            if first_key:
                ...
                #print(sub_content)
                sub_content = translate_subcontent_keys(key=key, sub_content=sub_content, translation_dict=imd_field_translation_dict)
                #print(sub_content)
    ...
    with open(in_file_path, 'r') as infile:
        ...
        for line in infile:
            ...
            #BlockEnds
                ...
                block = []
                first_key = True
                if(key == "TIMESTEP"): current_step +=1
                continue
    ...
    with open(in_path, "r") as infile:
        ...
        for block_title in block_lines:
            ...
            if(block_title in ["TITLE", "ENEVERSION"]):
                ...
                raise ValueError("FOUND A UKNOWN BLOCK! - PLEASE IMPLEMENT IT\n\t", block_title,"\n\t", "\t".join(block_lines[block_title]))
    return block_set
