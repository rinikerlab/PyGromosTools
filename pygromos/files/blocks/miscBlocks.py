...
class RESIDUENAMELIB(_general_blocks._generic_gromos_block):
    ...
    def __init__(self, content:Union[str, List[str], Dict[str, Dict[str, str]]]):
        ...
        if(isinstance(content, list)):
            ...
            raise ValueError("Class<"+str(self.__class__.__name__)+">:\t did not get appropriate data type.\n got: "+str(type(content))+"\n"+str(content))
...
class ATOMNAMELIB(_general_blocks._generic_gromos_block):
    ...
    def read_content_from_str(self, content):
        self.pdb_top = defaultdict(list)
        tuples = [(resn, pdb, top) for resn, pdb, top in list(map(lambda x: x.strip().split(), filter(lambda x: not x.startswith("#"), content)))]
        pdb_top = defaultdict(dict)
